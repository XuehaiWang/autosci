...
class SkillEngine:
    ...
    def __init__(self, skill_dirs: list[str], include_builtin: bool = True):
        ...
        if include_builtin:
            # Built-in skills are subdirectories of the skills/ package directory
            builtin_dir = os.path.dirname(__file__)
            if builtin_dir not in self.skill_dirs:
                self.skill_dirs.append(builtin_dir)
        self.skills: dict[str, Skill] = {}
        self._discover()

    def _discover(self) -> None:
        """Scan skill directories for skill files.

        Supports two layouts:
        1. Folder-based: <dir>/<name>/skill.md (preferred)
        2. Flat file:    <dir>/<name>.md (legacy)
        """
        for skill_dir in self.skill_dirs:
            ...
            for entry in sorted(os.listdir(skill_dir)):
                ...
                # Flat file: <name>.md (legacy)
                if entry.endswith(".md"):
                    self._load_skill_file(entry_path)

        if self.skills:
            logger.info(f"Discovered {len(self.skills)} skills")

    def _load_skill_file(self, filepath: str) -> None:
