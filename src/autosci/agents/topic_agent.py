"""TopicExplorationAgent — used when only a broad research topic is given.

Searches literature, brainstorms ideas, selects promising directions,
and produces Research Questions + Claims. Needs web tools so it is
defined in Python rather than YAML.

The task_given variant is defined in agents/templates/task_understanding.yaml.
"""
...
class TopicExplorationAgent(BaseAgent):
    """Agent for broad topic exploration — needs web tools, not suitable for YAML."""

    name = "topic_exploration"
    role = "Broad topic exploration: literature search, idea generation, RQ and claim synthesis"
    tools = ["web_search", "web_fetch", "read_file", "write_file", "list_dir", "glob", "grep"]
    max_iterations = 40

    def get_system_prompt(self, available_agents: list[dict] = None) -> str:
        return _TOPIC_ONLY_PROMPT


agent_registry.register(TopicExplorationAgent)
